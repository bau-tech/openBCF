...
- `bcf_client.py`/`oauth.py` — a BCF REST client and OAuth2 Authorization Code (RFC 6749, with
  ...
  bcf.chladny.de's actual flat) accommodation for viewpoint camera/selection/snapshot data (see
...
calls against the project's own test server (bcf.chladny.de) - version discovery, auth discovery,
...
bcf.chladny.de: sign-in, listing the account's actual projects, creating a topic, adding a
