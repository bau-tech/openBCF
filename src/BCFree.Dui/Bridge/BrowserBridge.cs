...
public sealed class BrowserBridge : IBrowserBridge
{
    ...
    {
        var script =
            $"window.__bcfreeDuiReceiveResult && window.__bcfreeDuiReceiveResult({JsonSerializer.Serialize(requestId)}, {(isError ? "true" : "false")}, {JsonSerializer.Serialize(payloadJson)})";
    ...
    public void Send(string eventName, object? payload)
    {
        ...
        var script =
            $"window.__bcfreeDuiReceiveEvent && window.__bcfreeDuiReceiveEvent({JsonSerializer.Serialize(FrontendBoundName)}, {JsonSerializer.Serialize(eventName)}, {JsonSerializer.Serialize(payloadJson)})";
        _scriptExecutor.ExecuteScript(script);
    }
}
