...
public static class BcfArchiveSync
{
    ...
    {
        var topics = await client.GetTopicsAsync(versionId, project.ProjectId, cancellationToken).ConfigureAwait(false);
        var topicFolders = new List<BcfTopicFolder>();

        foreach (var topic in topics)
        {
            var comments = await client.GetCommentsAsync(versionId, project.ProjectId, topic.Guid, cancellationToken).ConfigureAwait(false);
            var viewpoints = await client.GetViewpointsAsync(versionId, project.ProjectId, topic.Guid, cancellationToken).ConfigureAwait(false);
            ...
            foreach (var viewpoint in viewpoints)
            {
                ...
                try
                {
                    var snapshotBytes = await client.GetSnapshotAsync(versionId, project.ProjectId, topic.Guid, viewpoint.Guid, cancellationToken).ConfigureAwait(false);
        ...
        IBcfServerClient client, string versionId, string projectId, BcfDocument document, CancellationToken cancellationToken = default)
    {
        ...
        {
            var createdTopic = await client.CreateTopicAsync(versionId, projectId, topicFolder.Markup.Topic, cancellationToken).ConfigureAwait(false);
            ...
            foreach (var viewpointEntry in topicFolder.Viewpoints)
            {
                ...
                var createdViewpoint = await client.CreateViewpointAsync(
                    versionId, projectId, createdTopic.Guid, viewpointEntry.Value, snapshotBytes, cancellationToken).ConfigureAwait(false);
            ...
            foreach (var comment in topicFolder.Markup.Comments)
            {
                ...
                await client.CreateCommentAsync(versionId, projectId, createdTopic.Guid, remapped, cancellationToken).ConfigureAwait(false);
            }
        }
    }
}
