...
public sealed class BcfSessionBinding : IBinding
{
    ...
    public BcfSessionBinding(IBrowserBridge parent, UIApplication? uiApplication)
    {
        Parent = parent;
    ...
    private Task<BcfProject?> PickProjectAsync(IReadOnlyList<BcfProject> projects, string? previousProjectId)
    {
        ...
        // await below would run synchronously, inline, on that same call - reentering Revit's
        // dispatcher in a way it doesn't reliably pump, which is what made Connect() hang
        // indefinitely after picking a project.
    ...
    private string ResolveModelKey()
    {
        var document = _uiApplication?.ActiveUIDocument?.Document
            ?? throw new InvalidOperationException("Open a Revit document before connecting to a BCF server.");

        return string.IsNullOrEmpty(document.PathName) ? document.Title : document.PathName;
    }
}
